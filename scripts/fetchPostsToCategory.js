...
function writePosts(postList) {
  postList.forEach((post) => {
    ...
    const { body, bodyText, bodyHTML, category, ...frontmatter } = post
    const contentBody = config.formatPostBody(post)
    const categoryDir = join(config.outputDir, config.postSubDir, category)
    Deno.mkdirSync(categoryDir, {
	    recursive: true,
    })	    
    ...
    Deno.writeTextFileSync(
      join(config.outputDir, config.postSubDir, category, `${filename}.md`),
      content
    )
  })
}
